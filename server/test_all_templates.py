...
# ALL captions from user's video (complete)
...
# Test all templates
templates = ["classic", "neon", "bold", "minimal", "gradient"]

print("🎬 Testing all caption templates!\n")
...
for template in templates:
    ...
    # Wait for completion
    while True:
        status = requests.get(f"{SERVER_URL}/status/{job_id}").json()
    ...
    output_file = f"output_{template}.mp4"
    with open(output_file, 'wb') as f:
        f.write(response.content)
    
    print(f"   ✅ Saved to: {output_file}\n")

print("🎉 All templates generated! Compare:")
for template in templates:
    print(f"   • output_{template}.mp4")
